...
class Hexperience(object):
  def __init__(self, batch_size, memory_size, n_features, n_classes, observation_dims=[15]):
    ...
    self.unmissing = np.empty([self.memory_size, self.n_features],
                                dtype=np.uint8)
    self.actions_m = np.empty(self.memory_size, dtype=np.uint8)
    self.actions_w = np.empty(self.memory_size, dtype=np.uint8)
  ...
  def add(self, observed, acquired, reward, action_m, action_w, terminal, label):
    '''
    ...
    self.actions_m[self.current] = action_m
    self.actions_w[self.current] = action_w
  ...
  def sample(self):
    ...
    actions_m = self.actions_m[indices]
    actions_w = self.actions_w[indices]
    rewards = self.rewards[indices]
    terminals = self.terminals[indices]
    unmissing = self.unmissing[indices]
    unmissing_pre = self.unmissing[(indices - 1) % self.count]
    labels = self.labels[indices]
    return self.prestates, unmissing_pre, actions_m, actions_w, rewards,\
